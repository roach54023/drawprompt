...
// ─── Daily limit (shared key with HomeClient) ──────────────────────────────
const DAILY_LIMIT = 5;
...
function buildPrompt(difficulty: Difficulty): GeneratedPrompt {
  ...
  const parts: string[] = [];
  if (values.mood) parts.push(values.mood[0].toUpperCase() + values.mood.slice(1));
  ...
  const handleSave = () => {
    ...
    try {
      ...
      if (saved) {
        ...
        localStorage.setItem("saved_prompts", JSON.stringify([{ text: prompt.text, selections: prompt.selections, savedAt: new Date().toISOString(), difficulty }, ...list]));
        setSaved(true);
      }
    } catch {}
  };
    ...
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "48px 24px 80px" }}>
      ...
      <div style={{ textAlign: "center", marginBottom: 48 }}>
        ...
        <p style={{ fontSize: 15, color: "#6b5d4a", maxWidth: 420, margin: "0 auto", lineHeight: 1.7 }}>
          Every click is a completely new creative brief — mood, subject, palette, and style, all randomised. 5 prompts per day, free.
        </p>
      </div>
      ...
      <p style={{ textAlign: "center", fontSize: 12, color: "#c4b49a", marginBottom: 20 }}>{cfg.desc}</p>
          ...
          <div style={{ display: "flex", gap: 4 }}>
            {Array.from({ length: DAILY_LIMIT }).map((_, i) => (
              <span key={i} style={{
                ...
                background: i < usageToday ? (exhausted ? "#b85a5a" : "#c4714a") : "#e8e0d0",
      ...
      {exhausted && !prompt ? (
        <div className="card" style={{ padding: "48px 32px", textAlign: "center", border: "1.5px dashed #e8e0d0", background: "#faf8f4" }}>
          <p style={{ fontSize: 15, fontWeight: 600, color: "#2c2416", marginBottom: 8 }}>You&apos;ve used all 5 prompts for today</p>
          <p style={{ fontSize: 13, color: "#a8967e", marginBottom: 20 }}>Come back tomorrow for 5 more.</p>
        ...
        <>
          <div key={animKey} className={prompt ? "animate-scale-in" : ""}>
            {prompt ? (
              <PromptCard prompt={prompt} isSaved={saved} onSave={handleSave} difficulty={difficulty} difficultyColor={cfg.color} />
            ) : (
              <div className="card" style={{ padding: "48px 32px", textAlign: "center", border: "1.5px dashed #e8e0d0", background: "#faf8f4" }}>
      ...
      {prompt && (
        <div style={{ marginTop: 40 }}>
          ...
          <DimensionSelector selections={prompt.selections} activeKeys={cfg.keys} readOnly />
        </div>
      )}
      ...
      <div style={{ marginTop: 64, paddingTop: 40, borderTop: "1px solid #e8e0d0" }}>
        <h2 className="font-serif" style={{ fontSize: 18, fontWeight: 600, color: "#2c2416", marginBottom: 14 }}>
          What makes a good random drawing prompt?
        </h2>
        <p style={{ fontSize: 13, color: "#7a6a56", lineHeight: 1.8, marginBottom: 12 }}>
          A great random drawing prompt isn&apos;t just a random word — it&apos;s a complete creative brief. This generator combines six dimensions (theme, subject, mood, color palette, style, and challenge) to give you a prompt that&apos;s specific enough to be useful but open enough to interpret your own way.
        </p>
        <p style={{ fontSize: 13, color: "#7a6a56", lineHeight: 1.8 }}>
          Unlike AI-generated prompts, every combination here is hand-curated and guaranteed to be coherent and actually drawable. No hallucinated nonsense, no impossible scenes — just clean, inspiring creative briefs.
        </p>
      </div>
    </div>
  );
